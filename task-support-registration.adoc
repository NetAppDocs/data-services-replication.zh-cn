...
summary: 在通过 NetApp 技术支持开立支持案例之前，您需要将 NetApp Support Site 帐户添加到 Console 帐户，然后注册以获取支持。 
---
= 注册以获得支持
:allow-uri-read: 
:icons: font
...
[role="lead"]
要获得特定于 NetApp Console 及其存储解决方案和数据服务的技术支持，需要进行支持注册。此外，还需要进行支持注册才能为 Cloud Volumes ONTAP 系统启用关键工作流程。
...
. 完成 <<拥有 NSS 帐户的现有客户>> 下的步骤，将您的新 NSS 帐户与 Console 登录相关联。
...
拥有 NetApp 支持站点帐户后，请完成 <<拥有 NSS 帐户的现有客户>> 下的步骤，将该帐户与您的 Console 登录相关联。
